# nunatak - domain model

Glossary of nunatak: a zero-instrumentation profiling library that places
compute units on a roofline, diagnoses CPU/GPU/memory/network bottlenecks,
and has them explained by an LLM. This file is a glossary, not a spec: it
fixes the vocabulary, never the implementation.
...
performance vs ceilings) and its classification (memory-bound, core-bound,
latency-bound, imbalance...). Recomputed on demand from the measured pivot
