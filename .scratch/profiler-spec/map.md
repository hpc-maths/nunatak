...
- Stratégie de test et CI sur du hardware hétérogène (CPU variés, GPU des deux vendeurs, cluster MPI, Mac). Le ticket 07 en alourdit la charge : le symboliseur et l'analyse statique de boucle doivent être validés par microarchitecture, et l'attribution doit l'être sur des binaires volontairement dégradés (sans `-g`, strippés, sans frame pointers, sans `-lineinfo`). Le ticket 13 en fournit la pièce maîtresse : un **corpus de binaires figés** avec leurs sorties attendues, rejoué à chaque nouvelle version d'outil orchestré. Reste à spécifier le reste de la stratégie, et la matrice de build des wheels.
- Surface CLI complète et cohérente (`run`, `doctor`, `compare`, régénération de rapport, `--strict`, sortie JSON) - aucun ticket ne la couvre aujourd'hui, alors que `doctor` a pris énormément de responsabilités aux tickets 04, 05, 07 et 13, et que `compare` et `--strict` sont entrés en v1 aux tickets 07 et 13. C'est le manque le plus visible de la carte.
