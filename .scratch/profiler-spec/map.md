...
- Cadrage acté pendant le charting (session du 2026-07-23) :
  ...
  - Conséquence de packaging à répercuter au ticket 13 : le ticket 07 fait embarquer dans la wheel, en plus du noyau de calibration, un **symboliseur permissif et les briques LLVM** (désassembleur + modèles d'ordonnancement par microarchitecture), ces dernières servant à la fois la symbolisation et l'analyse statique de boucle qui reprend les cas d'usage de MAQAO sans MAQAO.
...
- Stratégie de test et CI sur du hardware hétérogène (CPU variés, GPU des deux vendeurs, cluster MPI, Mac). Le ticket 07 en alourdit la charge : le symboliseur et l'analyse statique de boucle doivent être validés par microarchitecture, et l'attribution doit l'être sur des binaires volontairement dégradés (sans `-g`, strippés, sans frame pointers, sans `-lineinfo`).
- Surface CLI complète et cohérente (`run`, `doctor`, `compare`, régénération de rapport) - aucun ticket ne la couvre aujourd'hui, alors que `doctor` a pris beaucoup de responsabilités aux tickets 04, 05 et 07, et que `compare` est entré en v1 au ticket 07.
- Contenu et structure de la spec finale elle-même (l'assemblage) - graduable quand la majorité des décisions seront prises.
- Gestion des erreurs et dégradation gracieuse quand un collecteur manque sur la machine cible. Le ticket 07 en a réglé le volet attribution (niveaux de résolution, absence de piles, source introuvable) ; reste le volet « collecteur entièrement absent ».
